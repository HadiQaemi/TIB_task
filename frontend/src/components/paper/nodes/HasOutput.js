...
const HasOutput = ({ data, mykey, styles }) => {
    const key = mykey
    const has_output = helper.checkType("has_output", data, 1)
    const label = helper.checkType("label", has_output, 1)
    const source_table = helper.checkType("source_table", has_output, 1)
    const has_expression = helper.checkType("has_expression", has_output, 1)
    const has_part = helper.checkType("has_part", has_output, 1)
    const source_url = helper.checkType("source_url", has_output, 1)
    const comment = helper.checkType("comment", has_output, 1)

    const has_characteristic = helper.checkType("has_characteristic", has_output, 1)
    ...
    return (
        <React.Fragment>
            <Row className="mx-0">
                <Col className="px-0">
                    <div className="border-card p-1" style={{ borderLeft: '10px solid #00b050' }}>
                        {label !== undefined ? (
                            <URLOrText button={'Output data'} color={'#00b050'} content={label ? label : ''} styles={styles} />
                        ) : (<></>)}
                        {source_url !== undefined ? (
                            <URLOrText button={label ? '' : 'Output data'} color={'#00b050'} content={`${source_url}`} styles={styles} />
                        ) : (<></>)}
                        {character !== '' ? (
                            <URLOrText button={(label || source_url) ? '' : 'Output data'} color={'#00b050'} content={`${character}`} styles={styles} />
                        ) : (<></>)}
                        {comment !== undefined ? (
                            <URLOrText button={(label || source_url || character) ? '' : 'Output data'} color={'#00b050'} content={`${comment}`} styles={styles} />
                        ) : (<></>)}
